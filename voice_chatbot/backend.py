...
async def get_models():
    ...
    try:
        ...
        # Handle both old dict response and new object response from ollama-python
        if hasattr(response, 'models'):
            ollama_models = [m.model for m in response.models]
        elif isinstance(response, dict):
            ollama_models = [m.get('name', '') for m in response.get('models', [])]
        else:
            ollama_models = []
            
        # Combine both lists and remove duplicates while preserving order
        all_models = list(dict.fromkeys(fallback_models + ollama_models))
        
        return {"models": all_models}
    except Exception as e:
        print(f"Error fetching ollama models: {e}")
        # Fallback list of common models in case ollama.list() fails
